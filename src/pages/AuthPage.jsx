 ...
 *
 * MUDANÇAS vs. app:
 * - Layout web: card centralizado em tela cheia com gradiente de fundo
 * - Em desktop: card de largura máxima 460px centralizado
 * - Em mobile: fullscreen igual ao app original
 * - Credenciais demo mantidas: admin / 1234
 */
  ...
  return (
    <div className="input-wrap">
      ...
      <div className="input-field-wrap">
        ...
        />
        {isPassword && (
          <button type="button" onClick={() => setShow(s => !s)} className="input-toggle">
...
function LoginForm({ onSwitch }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <Field label="Usuário" type="text" value={usuario}
        onChange={v => { setUsuario(v); setAuthError(''); }}
        placeholder="admin" icon="user" />
      <Field label="Senha" type="password" value={senha}
        onChange={v => { setSenha(v); setAuthError(''); }}
      ...
      <div style={{ background: 'var(--gold-dim)', border: '1px solid var(--gold-border)', borderRadius: 10, padding: '10px 14px' }}>
        <p style={{ fontSize: 10, fontWeight: 900, color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>
          Acesso Demo
        </p>
      ...
      <button type="submit" disabled={loading} className="btn btn-gold" style={{ width: '100%', marginTop: 4 }}>
        {loading
          ? <><Icon name="loader-2" size={16} className="anim-spin" /> Entrando...</>
      ...
      <p style={{ textAlign: 'center', fontSize: 13, color: 'var(--text-muted)' }}>
        ...
        <button type="button" onClick={onSwitch}
          style={{ background: 'none', border: 'none', color: 'var(--gold)', fontWeight: 900, cursor: 'pointer', fontSize: 13 }}>
          Cadastre-se
        </button>
      </p>
    </form>
...
function CadastroForm({ onSwitch }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <Field label="Telefone" value={telefone} onChange={v => { setTelefone(v); setAuthError(''); }} placeholder="(11) 99999-9999" icon="phone" />
      ...
      <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px' }}>
        <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>
          💡 <strong>Modo demo:</strong> sessão em memória. Dados perdidos ao recarregar.
        </p>
      </div>

      <button type="submit" disabled={loading} className="btn btn-gold" style={{ width: '100%' }}>
        {loading
          ? <><Icon name="loader-2" size={16} className="anim-spin" /> Criando conta...</>
...
export default function AuthPage() {
  ...
  return (
    <div style={{
      minHeight: '100vh',
      background: 'radial-gradient(ellipse at 60% 0%, rgba(245,200,66,0.07) 0%, transparent 60%), var(--bg)',
      ...
      <div style={{ width: '100%', maxWidth: 440 }} className="anim-scale-in">
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <img
            ...
            style={{ width: 72, height: 72, borderRadius: 18, objectFit: 'cover', border: '1px solid var(--gold-border)', marginBottom: 12 }}
            onError={e => { e.target.style.display = 'none'; }}
          />
          <p className="bebas" style={{ fontSize: 32, letterSpacing: '0.1em' }}>
            <span style={{ color: '#fff' }}>LIRA </span>
            <span className="shimmer-text">SNEAKERS</span>
          </p>
          <p style={{ fontSize: 11, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.2em', fontWeight: 700, marginTop: 4 }}>
            Sneaker Spa · Programa de Fidelidade
          </p>
        </div>
        ...
        <div className="card">
          ...
          <div className="tabs-wrap" style={{ marginBottom: 24 }}>
            {[['login', 'Entrar'], ['cadastro', 'Criar conta']].map(([id, lbl]) => (
              <button key={id} onClick={() => setModo(id)} className={`tab-btn ${modo === id ? 'active' : ''}`}>
                {lbl}
              </button>
            ))}
          </div>
          ...
          {modo === 'login'
            ? <LoginForm onSwitch={() => setModo('cadastro')} />
            : <CadastroForm onSwitch={() => setModo('login')} />
          }
        </div>
      </div>
    </div>
