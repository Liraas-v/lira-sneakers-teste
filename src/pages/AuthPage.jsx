...
function CadastroForm({ onSwitch }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (senha.length < 4) {
      setAuthError("Senha mínima: 4 caracteres.");
      return;
    }
    if (senha !== confirm) {
  ...
  return (
    ...
    >
      ...
      >
        {loading ? (
          <>
            <Icon name="loader-2" size={16} className="anim-spin" /> Criando
            conta...
          </>
        ) : (
          <>
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background:
          "radial-gradient(ellipse at 60% 0%, rgba(245,200,66,0.07) 0%, transparent 60%), var(--bg)",
        display: "flex",
        alignItems: "center",
