/**
 * src/pages/SuportePage.jsx
 *
 * MUDANÇAS vs. app:
 * - Layout 2 colunas no desktop: FAQ à esquerda | canais de contato à direita
 * - Em mobile: tab switcher original mantido (FAQ / Falar Conosco)
 * - window.open com _blank para não navegar para fora do site
...
function FaqItem({ item, index }) {
  ...
  return (
    <div
      style={{
        borderRadius: 12,
        border: `1px solid ${open ? 'rgba(245,200,66,.3)' : 'var(--border)'}`,
        background: open ? 'rgba(245,200,66,.04)' : 'var(--surface)',
        transition: 'all 0.2s',
    ...
    >
      ...
      >
        <span style={{ fontSize: 13, fontWeight: 700, color: open ? 'var(--gold)' : 'var(--text)', lineHeight: 1.4 }}>
          {item.pergunta}
        </span>
        <Icon
          name={open ? 'chevron-up' : 'chevron-down'}
          size={16}
          style={{ color: open ? 'var(--gold)' : 'var(--text-dim)', flexShrink: 0, marginTop: 2 }}
        />
      </button>
...
export default function SuportePage() {
  const { user } = useApp();
  // Tab usado apenas no mobile (≤768px)
  ...
  const CANAIS = [
    {
      icon: 'message-circle', titulo: 'WhatsApp',
      desc: 'Seg–Dom, 9h–18h · Resposta em até 1h',
      cor: 'var(--green)', bg: 'var(--green-dim)', border: 'var(--green-border)',
      msg: `Olá! Sou o(a) ${user?.nome ?? 'cliente'} e preciso de ajuda.`,
    },
    {
      icon: 'instagram', titulo: 'Instagram',
      desc: '@lirasneakerslab · DMs abertas',
      cor: '#e1306c', bg: 'rgba(225,48,108,.08)', border: 'rgba(225,48,108,.25)',
      url: 'https://instagram.com/lirasneakers',
    },
    {
      icon: 'map-pin', titulo: 'Endereço',
      desc: 'Consultar localização via WhatsApp',
      cor: 'var(--gold)', bg: 'var(--gold-dim)', border: 'var(--gold-border)',
      msg: 'Olá! Gostaria de saber o endereço para deixar meu tênis.',
    },
  ];

  const MENSAGENS_RAPIDAS = [
    ['Orçamento',        `Olá! Gostaria de solicitar um orçamento para ${user?.nome ?? 'meu serviço'}.`],
  ...
  // Painel de contato (reutilizado nos dois layouts)
  const PainelContato = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {CANAIS.map((canal, i) => (
        <button
          key={canal.titulo}
          onClick={() => canal.url ? window.open(canal.url, '_blank') : abrirWhatsApp(canal.msg)}
          style={{
            display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px',
            background: canal.bg, border: `1px solid ${canal.border}`,
      ...
      <div className="card" style={{ padding: 18 }}>
        <p style={{ fontSize: 12, fontWeight: 900, color: 'var(--text)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>
          Mensagem rápida
        </p>
      ...
      <div className="card" style={{ padding: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
          ...
          <p style={{ fontSize: 12, fontWeight: 900, color: 'var(--text)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      <div className="page-wrap">
        ...
        <div
          className="suporte-grid anim-fade-up"
          style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 28, alignItems: 'start' }}
        >
          {/* FAQ */}
          <div>
            <div style={{ marginBottom: 20 }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
                {FAQ_ITEMS.length} perguntas · clique para expandir
              </p>
            </div>
        ...
        <style>{`
          @media (max-width: 768px) {
            ...
            .suporte-mobile-tabs { display: block !important; }
            .suporte-desktop-col { display: block; }
          }
        `}</style>
      </div>
