...
const DEMO_PEDIDOS = [
  { id: 1, nome: 'Lucas M.', tel: '(11) 98001-2345', servicos: 'Limpeza Completa', total: 70, pago: 70, statusServico: 'concluido', data: '10/03/2025', obs: 'Air Jordan 1 branco' },
  { id: 2, nome: 'Fernanda R.', tel: '(11) 97002-3456', servicos: 'Pintura Entressola + Impermeabilização', total: 125, pago: 125, statusServico: 'concluido', data: '25/03/2025', obs: 'Yeezy 350' },
  { id: 3, nome: 'Gabriel S.', tel: '', servicos: 'Impermeabilização', total: 35, pago: 0, statusServico: 'andamento', data: '01/04/2025', obs: 'New Balance 574' },
  { id: 4, nome: 'Camila T.', tel: '', servicos: 'Restauração de Camurça', total: 70, pago: 50, statusServico: 'andamento', data: '05/04/2025', obs: 'Puma suede verde' },
  { id: 5, nome: 'Rafael B.', tel: '(11) 96003-4567', servicos: 'Limpeza Premium + Colagem', total: 160, pago: 100, statusServico: 'andamento', data: '10/04/2025', obs: 'Balenciaga Triple S' },
  { id: 6, nome: 'Laura S.', tel: '', servicos: 'Limpeza Completa + Pintura Cabedal', total: 170, pago: 0, statusServico: 'aguardando', data: '15/04/2025', obs: '' },
];
...
function ProgressBar({ value, max }) {
  ...
  return (
    <div style={{ height: 5, background: 'var(--surface-3)', borderRadius: 99, overflow: 'hidden', marginTop: 5 }}>
      <div style={{ height: '100%', width: `${p}%`, background: 'linear-gradient(90deg,#d4a800,var(--gold))', borderRadius: 99, transition: 'width 0.4s' }} />
    </div>
  );
}
...
function PedidoModal({ pedido, onSave, onClose }) {
  const [form, setForm] = useState({
    nome: '', tel: '', servicos: '', total: '', pago: '',
    statusServico: 'andamento', data: today(), obs: '',
    ...pedido,
  });
  ...
  return (
    <div className="modal-backdrop" onClick={e => { if (e.target.classList.contains('modal-backdrop')) onClose(); }}>
      <div className="modal">
        <h2 className="modal-title bebas">{pedido ? 'EDITAR PEDIDO' : 'NOVO PEDIDO'}</h2>

        <div className="form-row">
          <div className="form-group">
            <label className="form-label">Nome do Cliente *</label>
            <input type="text" value={form.nome} onChange={e => up('nome', e.target.value)} placeholder="Ex: Laura Silva" />
          </div>
          <div className="form-group">
            <label className="form-label">Telefone</label>
            <input type="text" value={form.tel} onChange={e => up('tel', e.target.value)} placeholder="(11) 99999-9999" />
          </div>
        </div>

        <div className="form-group">
          <label className="form-label">Serviços *</label>
          <input type="text" value={form.servicos} onChange={e => up('servicos', e.target.value)} placeholder="Ex: Limpeza Completa + Pintura Entressola" />
        </div>

        <div className="form-row">
          <div className="form-group">
            <label className="form-label">Valor Total (R$) *</label>
            <input type="number" value={form.total} onChange={e => up('total', e.target.value)} placeholder="160" min="0" step="0.01" />
          </div>
          <div className="form-group">
            <label className="form-label">Já Recebido (R$)</label>
            <input type="number" value={form.pago} onChange={e => up('pago', e.target.value)} placeholder="0" min="0" step="0.01" />
          </div>
        </div>
        ...
        {total > 0 && (
          <div style={{ marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>
              <span style={{ color: 'var(--green)' }}>Recebido: R$ {pago.toFixed(2)}</span>
              <span style={{ color: diff > 0 ? 'var(--red)' : 'var(--green)' }}>Falta: R$ {diff.toFixed(2)} ({p}%)</span>
            </div>
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <select value={form.statusServico} onChange={e => up('statusServico', e.target.value)}>
          ...
          <div className="form-group">
            ...
            <input type="text" value={form.data} onChange={e => up('data', e.target.value)} placeholder="DD/MM/AAAA" />
          </div>
        </div>

        <div className="form-group">
          <label className="form-label">Observações</label>
          <textarea value={form.obs} onChange={e => up('obs', e.target.value)} placeholder="Modelo do tênis, cor, detalhes..." style={{ height: 60 }} />
        </div>

        <div className="modal-actions">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-gold" onClick={handleSave}>Salvar Pedido</button>
        </div>
      </div>
    </div>
  );
...
function PagamentoModal({ pedido, onSave, onClose }) {
  ...
  const p = pedido.total > 0 ? Math.min(100, Math.round((novoPago / pedido.total) * 100)) : 0;

  const handleSave = () => {
    if (!novo) { alert('Informe o valor recebido.'); return; }
    onSave(novo);
  };
      ...
      <div className="modal" style={{ maxWidth: 420 }}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--border)', fontSize: 12 }}>
            ...
            <span style={{ fontWeight: 700, color: 'var(--green)' }}>R$ {pedido.pago}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', fontSize: 12 }}>
            ...
            <span style={{ fontWeight: 700, color: 'var(--red)' }}>R$ {falta(pedido)}</span>
          </div>
        ...
        <div className="form-group">
          ...
          <input type="number" value={valor} onChange={e => setValor(e.target.value)} placeholder="0" min="0" step="0.01" autoFocus />
        </div>

        {novo > 0 && (
          <div style={{ marginTop: 8, marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>
              <span style={{ color: 'var(--green)' }}>Novo total: R$ {novoPago.toFixed(2)}</span>
              <span style={{ color: novaFalta > 0 ? 'var(--red)' : 'var(--green)' }}>
                {novaFalta > 0 ? `Falta: R$ ${novaFalta.toFixed(2)}` : 'Quitado! ✓'}
              </span>
            </div>
        ...
        <div className="modal-actions">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn btn-gold" onClick={handleSave}>Confirmar Pagamento</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="modal-backdrop" onClick={e => { if (e.target.classList.contains('modal-backdrop')) onClose(); }}>
      <div className="modal">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: 13 }}>
          ...
          <span style={{ fontWeight: 900, fontSize: 16, color: 'var(--gold)' }}>R$ {pedido.total}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)', fontSize: 13 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: 13 }}>
          ...
          <span style={{ fontWeight: 900, color: f > 0 ? 'var(--red)' : 'var(--green)' }}>
            {f > 0 ? `R$ ${f}` : 'Quitado ✓'}
          </span>
        </div>
        <ProgressBar value={pedido.pago} max={pedido.total} />
        <div style={{ textAlign: 'right', fontSize: 10, color: 'var(--text-dim)', marginTop: 4, marginBottom: pedido.obs ? 12 : 0 }}>
          {pct(pedido)}% recebido
        </div>
        {pedido.obs && (
          <div style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '10px 14px', fontSize: 12, color: 'var(--text-muted)', marginTop: 10 }}>
            {pedido.obs}
          </div>
        )}
        <div className="modal-actions">
          <button className="btn btn-ghost" onClick={onClose}>Fechar</button>
          <button className="btn btn-gold" onClick={onEdit}>✎ Editar</button>
        </div>
      </div>
    </div>
  );
...
function Dashboard({ pedidos }) {
  ...
  const ticketMedio   = pedidos.length ? Math.round(totalReceita / pedidos.length) : 0;
  ...
  const chartOptions = {
    ...
    scales: {
      ...
      y: { ticks: { color: '#555', font: { size: 10 }, callback: v => `R$${v}` }, grid: { color: '#1e1e1e' } },
    },
  };
  ...
  return (
    <div>
      ...
      <div className="metrics-grid">
        <MetricCard label="Faturamento Total" value={`R$ ${totalReceita.toLocaleString('pt-BR')}`} sub={`${pedidos.length} pedidos`} color="gold" />
        <MetricCard label="Total Recebido" value={`R$ ${totalRecebido.toLocaleString('pt-BR')}`} sub={`${totalReceita > 0 ? Math.round((totalRecebido / totalReceita) * 100) : 0}% do faturamento`} color="green" />
        <MetricCard label="A Receber" value={`R$ ${totalFalta.toLocaleString('pt-BR')}`} sub={`${pedidos.filter(p => falta(p) > 0).length} pedidos pendentes`} color="red" />
        <MetricCard label="Ticket Médio" value={`R$ ${ticketMedio}`} sub="por pedido" color="neutral" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16, marginBottom: 16 }}>
        <div className="card">
          ...
          <div style={{ padding: '0 16px 16px' }}>
            ...
            <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
              {[['#f5c842', 'Faturado'], ['#4ade80', 'Recebido']].map(([c, l]) => (
        ...
        <div className="card">
          <div className="card-header"><span className="card-title">Status de Pagamentos</span></div>
          <div style={{ padding: '0 16px 16px' }}>
            <div style={{ position: 'relative', height: 140 }}>
              <Doughnut data={statusData} options={donutOptions} />
            </div>
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 10, flexWrap: 'wrap' }}>
              {[['#4ade80','Pago', statusCounts.pago], ['#facc15','Parcial', statusCounts.parcial], ['#f87171','Pendente', statusCounts.pendente]].map(([c, l, n]) => (
                <span key={l} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--text-muted)' }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: c, display: 'inline-block' }} />{l} ({n})
                </span>
              ))}
            </div>
          </div>
      ...
      <div className="card">
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              {['Cliente','Serviços','Total','Pago','Pagamento','Serviço'].map(h => (
                <th key={h} style={{ fontSize: 9, fontWeight: 900, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.12em', padding: '8px 14px', textAlign: 'left', borderBottom: '1px solid var(--border)' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {recentes.map(p => (
              <tr key={p.id} style={{ cursor: 'default' }}>
                <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 700 }}>{p.nome}</td>
                <td style={{ padding: '10px 14px', fontSize: 11, color: 'var(--text-muted)', maxWidth: 160, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.servicos}</td>
                <td style={{ padding: '10px 14px', fontSize: 12 }}>R$ {p.total}</td>
                <td style={{ padding: '10px 14px', fontSize: 12, color: falta(p) > 0 ? 'var(--yellow)' : 'var(--green)' }}>R$ {p.pago}</td>
                <td style={{ padding: '10px 14px' }}><Badge type={statusPagamento(p)} /></td>
                <td style={{ padding: '10px 14px' }}><Badge type={p.statusServico} /></td>
              </tr>
            ))}
          </tbody>
        </table>
...
// ── Seção: Pedidos ───────────────────────────────────────────────
function Pedidos({ pedidos, onNew, onEdit, onDelete, onPagar, onDetail, onQuickAdd }) {
  const [quickText, setQuickText] = useState('');
  const [filtro, setFiltro]       = useState('');

  const handleQuick = () => {
    if (!quickText.trim()) { alert('Digite as informações do pedido.'); return; }
  ...
  return (
    <div>
      {/* Quick Add */}
      <div style={{ background: 'var(--surface)', border: '1px dashed var(--gold-border)', borderRadius: 'var(--radius)', padding: '16px 18px', marginBottom: 20 }}>
        <div style={{ fontSize: 10, fontWeight: 900, color: 'var(--gold)', textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <input
            ...
            style={{ flex: 1, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', color: 'var(--text)', fontFamily: 'var(--font-body)', fontSize: 13, padding: '10px 14px', outline: 'none' }}
          />
          <button className="btn btn-gold" onClick={handleQuick}>Adicionar</button>
        </div>
        <div style={{ fontSize: 10, color: 'var(--text-dim)', marginTop: 7 }}>
          Formato sugerido: Nome — Serviços — R$ Valor · Pressione Enter para confirmar
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <select
              ...
              style={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', color: 'var(--text)', padding: '5px 10px', fontSize: 11 }}
            >
            ...
            <button className="btn btn-gold btn-sm" onClick={onNew}>+ Novo</button>
          </div>
        </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Data','Cliente','Serviços','Total','Recebido','Falta','Status','Ações'].map(h => (
                  <th key={h} style={{ fontSize: 9, fontWeight: 900, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.12em', padding: '8px 14px', textAlign: 'left', borderBottom: '1px solid var(--border)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {[...lista].reverse().map(p => {
                ...
                return (
                  <tr key={p.id}>
                    <td style={{ padding: '10px 14px', fontSize: 11, color: 'var(--text-dim)' }}>{p.data}</td>
                    <td style={{ padding: '10px 14px', fontSize: 12, fontWeight: 700 }}>{p.nome}</td>
                    <td style={{ padding: '10px 14px', fontSize: 11, color: 'var(--text-muted)', maxWidth: 150, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.servicos}</td>
                    <td style={{ padding: '10px 14px', fontSize: 12 }}>R$ {p.total}</td>
                    <td style={{ padding: '10px 14px', fontSize: 12, color: 'var(--green)' }}>R$ {p.pago}</td>
                    <td style={{ padding: '10px 14px', fontSize: 12, color: f > 0 ? 'var(--red)' : 'var(--green)', fontWeight: 900 }}>
                      {f > 0 ? `R$ ${f}` : '—'}
                    </td>
                    <td style={{ padding: '10px 14px' }}><Badge type={statusPagamento(p)} /></td>
                    <td style={{ padding: '8px 14px' }}>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button className="btn btn-ghost btn-sm" onClick={() => onDetail(p)}>Ver</button>
                        {f > 0 && <button className="btn btn-success btn-sm" onClick={() => onPagar(p)}>$ Pagar</button>}
...
function Financeiro({ pedidos, onPagar }) {
  ...
  const svcEntries = Object.entries(svcCount).sort((a, b) => b[1] - a[1]).slice(0, 5);
  ...
  const svcOptions = {
    ...
    plugins: { legend: { display: false } },
    scales: {
      x: { ticks: { color: '#555', font: { size: 10 }, stepSize: 1 }, grid: { color: '#1e1e1e' } },
      y: { ticks: { color: '#888', font: { size: 10 } }, grid: { display: false } },
    },
  };

  const comFalta = pedidos.filter(p => falta(p) > 0).sort((a, b) => falta(b) - falta(a));
  ...
  return (
    <div>
      ...
      <div className="metrics-grid">
        <MetricCard label="Faturamento Total" value={`R$ ${totalReceita.toLocaleString('pt-BR')}`} sub={`${pedidos.length} pedidos`} color="gold" />
        <MetricCard label="Total Recebido" value={`R$ ${totalRecebido.toLocaleString('pt-BR')}`} sub={`${totalReceita > 0 ? Math.round((totalRecebido / totalReceita) * 100) : 0}% do faturamento`} color="green" />
        <MetricCard label="A Receber" value={`R$ ${totalFalta.toLocaleString('pt-BR')}`} sub={`${comFalta.length} clientes pendentes`} color="red" />
        <MetricCard label="Ticket Médio" value={`R$ ${ticketMedio}`} sub="por pedido" color="neutral" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        ...
        <div className="card">
          <div className="card-header"><span className="card-title">A Receber por Cliente</span></div>
          <div style={{ maxHeight: 280, overflowY: 'auto' }}>
            {comFalta.length === 0
              ? <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-dim)', fontSize: 12 }}>Nenhuma pendência! 🎉</div>
              : comFalta.map(p => {
                const f = falta(p);
                return (
                  <div key={p.id} style={{ padding: '12px 16px', borderBottom: '1px solid #1e1e1e' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <b style={{ fontSize: 12 }}>{p.nome}</b>
                      <span style={{ color: 'var(--red)', fontWeight: 900, fontSize: 12 }}>- R$ {f}</span>
                    </div>
                    <div style={{ color: 'var(--text-muted)', fontSize: 11, marginBottom: 6, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.servicos}</div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--text-dim)', marginBottom: 4 }}>
                      <span>R$ {p.pago} de R$ {p.total}</span>
                      <span>{pct(p)}%</span>
                    </div>
        ...
        <div className="card">
          <div className="card-header"><span className="card-title">Serviços Mais Realizados</span></div>
          <div style={{ padding: '0 16px 16px' }}>
      ...
      <div className="card">
        <div className="card-header"><span className="card-title">Pendências de Pagamento</span></div>
        {comFalta.length === 0
          ? <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-dim)', fontSize: 12 }}>Sem pendências de pagamento! 🎉</div>
          : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  {['Cliente','Serviço','Total','Pago','Pendente','Ação'].map(h => (
                    <th key={h} style={{ fontSize: 9, fontWeight: 900, color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.12em', padding: '8px 14px', textAlign: 'left', borderBottom: '1px solid var(--border)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {comFalta.map(p => (
                  <tr key={p.id}>
                    ...
                    <td style={{ padding: '10px 14px', fontSize: 12 }}>R$ {p.total}</td>
                    <td style={{ padding: '10px 14px', fontSize: 12, color: 'var(--green)' }}>R$ {p.pago}</td>
                    <td style={{ padding: '10px 14px', fontSize: 12, color: 'var(--red)', fontWeight: 900 }}>R$ {falta(p)}</td>
                    <td style={{ padding: '8px 14px' }}>
                      <button className="btn btn-success btn-sm" onClick={() => onPagar(p)}>Registrar Pagamento</button>
                    </td>
                  </tr>
                ))}
              </tbody>
...
export default function AdminPage() {
  ...
  const salvarPedido = useCallback((data) => {
    if (data.id) {
      setPedidos(ps => ps.map(p => p.id === data.id ? { ...data, total: parseFloat(data.total), pago: parseFloat(data.pago) } : p));
    } else {
      setPedidos(ps => [...ps, { ...data, id: nextId, total: parseFloat(data.total), pago: parseFloat(data.pago) }]);
      setNextId(n => n + 1);
    }
    setModalNovo(false);
    setEditando(null);
  ...
  const registrarPagamento = useCallback((valor) => {
    setPedidos(ps => ps.map(p => {
      if (p.id !== pagando.id) return p;
      const novoPago = Math.min(p.total, p.pago + valor);
      return { ...p, pago: novoPago, statusServico: novoPago >= p.total ? 'concluido' : p.statusServico };
    }));
    setPagando(null);
  }, [pagando]);

  const quickAdd = useCallback((texto) => {
    ...
    setPedidos(ps => [...ps, { id: nextId, nome, tel: '', servicos, total, pago: 0, statusServico: 'andamento', data: today(), obs: '' }]);
    setNextId(n => n + 1);
  }, [nextId]);
  ...
  return (
    <>
      ...
      <div className="admin-shell">
        ...
        <aside className="admin-sidebar">
          <div className="admin-logo">
            <div className="bebas">LIRA <span style={{ color: 'var(--gold)' }}>ADMIN</span></div>
          ...
          <nav className="admin-nav">
            ...
            {SECOES.map(s => (
              ...
              >
                <span style={{ width: 16, textAlign: 'center', fontSize: 14 }}>{s.icon}</span>
                {s.label}
              </div>
            ))}
            <div className="admin-nav-label">Ações</div>
            <div className="admin-nav-item" onClick={() => setModalNovo(true)}>
              <span style={{ width: 16, textAlign: 'center', fontSize: 16 }}>+</span>
              Novo Pedido
            </div>
          </nav>
        </aside>
          ...
          <div className="admin-topbar">
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <span className="admin-topbar-title">
                {SECOES.find(s => s.id === secao)?.label?.toUpperCase() || 'ADMIN'}
              </span>
              <span className="admin-topbar-badge">
                {secao === 'dashboard' ? 'Visão Geral' : secao === 'pedidos' ? 'Gestão de Pedidos' : 'Receita & Pendências'}
              </span>
            </div>
            <button className="btn btn-gold btn-sm" onClick={() => setModalNovo(true)}>+ Novo Pedido</button>
          </div>
      ...
      {/* Modais */}
      {modalNovo && (
        <PedidoModal onSave={salvarPedido} onClose={() => setModalNovo(false)} />
      )}
      {editando && (
        <PedidoModal pedido={editando} onSave={salvarPedido} onClose={() => setEditando(null)} />
      )}
      {pagando && (
        <PagamentoModal pedido={pagando} onSave={registrarPagamento} onClose={() => setPagando(null)} />
      )}
      {detalhando && (
        <DetailModal
          pedido={detalhando}
          onEdit={() => { setEditando(detalhando); setDetalhando(null); }}
